        ...
        <div class="trend-card">
            <div class="trend-header">
                ...
                <div class="toggle-group">
                    <button type="button" class="toggle-btn active" id="btnViewDaily"
                        onclick="switchRevenuePeriod('daily')">
                        <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <div class="table-card">
            <div class="table-card-header">
                ...
                <div>
                    ...
                    <p style="font-size: 12px; color: var(--muted); margin: 2px 0 0;">
                        รายละเอียดสถิติยอดเงินที่อนุมัติแล้วและจำนวนครั้งการจองแยกตามรายเดือน</p>
                </div>
            </div>
            <div class="table-responsive">
                <table class="report-table">
                    ...
                    <tbody>
                        ...
                        @foreach ($monthlySummary as $row)
                            ...
                            <tr class="{{ $row['revenue'] > 0 ? 'highlight-row' : '' }}">
                                ...
                                <td
                                    style="text-align: right; font-weight: 700; color: {{ $row['revenue'] > 0 ? 'var(--maroon-900)' : 'var(--muted)' }};">
                    ...
                    <tfoot>
                        <tr>
                            <td style="font-weight: 800; color: var(--maroon-900);">รวมทั้งสิ้นตลอดปี {{ $currentYear }}
                            </td>
        ...
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                                ...
                                callbacks: {
                                    label: function(context) {
                                        ...
                                        const cnt = (currentPeriod === 'daily') ?
                                            ...
                                            (monthlyCounts[idx] || 0);
                                        return [
                                            ' รายได้: ฿' + val.toLocaleString('en-US', {
                ...
                function updateTrendStats(type) {
                    ...
                    document.getElementById('statPeriodTotal').innerText = '฿' + total.toLocaleString('en-US', {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2
                    });
                    document.getElementById('statPeriodAvg').innerText = '฿' + avg.toLocaleString('en-US', {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2
                    });
                    document.getElementById('statAvgLabel').innerText = (type === 'daily') ?
                        'เฉลี่ยต่อวัน (ที่มีรายได้)' : 'เฉลี่ยต่อเดือน (ที่มีรายได้)';
                    document.getElementById('statPeakLabel').innerText = (type === 'daily') ? 'วันที่รายได้สูงสุด' :
                        'เดือนที่รายได้สูงสุด';
                    document.getElementById('statPeriodPeak').innerText = (maxVal > 0) ? (peakLabel + ' (฿' + maxVal
                        .toLocaleString() + ')') : 'ยังไม่มีข้อมูล';
            ...
            const distinctFallbackColors = [
                '#0d9488', '#d97706', '#8b1e3f', '#2563eb', '#e11d48', '#7c3aed', '#059669', '#ea580c',
                '#0284c7', '#d946ef'
            ];
            ...
            const ctxCat = document.getElementById('categoryRevenueChart').getContext('2d'); window.catChart =
            new Chart(ctxCat, {
                type: 'doughnut',
                data: {
                        ...
                        tooltip: {
                            ...
                            callbacks: {
                                ...
                                label: function(context) {
                                    ...
                                    let total = context.dataset.data.reduce((a, b) => Number(a) +
                                        Number(b), 0);
                                    let percent = total > 0 ? ((value / total) * 100).toFixed(1) : 0;
                                    const index = context.dataIndex;
                                    const count = catCounts[index] || 0;
                                    return [
                                        ' รายได้: ฿' + value.toLocaleString('en-US', {
                                            minimumFractionDigits: 2,
                                            maximumFractionDigits: 2
                                        }) + ' (' + percent + '%)',
            ...
            const dressCategories = dressData.map(item => item.category ? item.category.category_name :
                'หมวดหมู่ทั่วไป');
            ...
            const uniqueCats = []; dressData.forEach((item, idx) => {
            ...
            const ctxDress = document.getElementById('topDressesChart').getContext('2d'); window.dressChart = new Chart(
                ctxDress, {
                    type: 'bar',
                    data: {
