        ...
        <form id="createDressForm" action="{{ route('owner.dresses.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <input type="number" step="0.01" name="deposit" value="{{ old('deposit', 100.00) }}" required class="form-control" placeholder="100.00">
                    <small style="color: var(--muted); font-size: 11px;">กำหนดค่ามัดจำมาตรฐาน 100 บาท (คืนให้ลูกค้าเมื่อคืนชุดและไม่มีความเสียหาย)</small>
                    @error('deposit')
                        <div class="error-feedback">{{ $message }}</div>
                    @enderror
                </div>
