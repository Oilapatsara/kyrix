...
<div class="rentals-wrapper">
        ...
        <div class="booking-card">
            <div class="booking-card-top">
                <div>
                    ...
                    <h3 style="font-family: 'Plus Jakarta Sans', monospace; font-size: 22px; color: var(--primary); font-weight: 800; margin: 0;">
                        {{ $rental->formatted_code }}
                    </h3>
                    <span style="font-size: 12px; color: #888;">วันที่จอง: {{ $rental->created_at->format('d/m/Y H:i') }} น.</span>
                </div>
                    ...
                    <div class="dress-row">
                        ...
                        <div style="flex: 1;">
                            <h4 style="font-size: 16px; font-weight: 700; margin-bottom: 4px;">
                                <a href="{{ route('products.show', $detail->product_id) }}" style="color: var(--text-main);">
            ...
            <div class="info-table-grid">
                <div class="info-col">
                    ...
                    <span class="info-val" style="color: var(--primary);">{{ date('d/m/Y', strtotime($rental->start_date)) }}</span>
                </div>
                <div class="info-col">
                    <span class="info-label">วันที่คืนชุด</span>
                    <span class="info-val" style="color: #b91c1c;">{{ date('d/m/Y', strtotime($rental->end_date)) }}</span>
                </div>
                <div class="info-col">
                    <span class="info-label">ยอดทั้งหมด (รวมมัดจำ)</span>
                    <span class="info-val">฿{{ number_format($rental->grand_total) }}</span>
                    <span style="font-size: 11px; color: var(--text-muted);">(มัดจำ: ฿{{ number_format($rental->deposit_amount) }})</span>
                </div>
                <div class="info-col">
                    ...
                    @if($rental->paid_amount < $rental->grand_total)
                        <span style="font-size: 11px; color: #dc2626;">(คงค้าง: ฿{{ number_format($rental->grand_total - $rental->paid_amount) }})</span>
                    @endif
                </div>
            </div>
    ...
    @empty
        <div style="background: #fff; border-radius: var(--radius-lg); border: 1px solid var(--border); padding: 70px 20px; text-align: center;">
            ...
            <p style="color: var(--text-muted); font-size: 14px; margin-bottom: 24px;">คุณสามารถเลือกดูชุดที่ชอบและกดจองเช่าชุดได้ทันที</p>
            <a href="{{ route('products.index') }}" class="btn btn-primary">
