...
<head>
    ...
    <title>@yield('title', 'KYRIX | ร้านเช่าชุดสายฝอ สายหวาน ชุดออกงาน')</title>
    ...
    <style>
        ...
        * {
        ...
        /* ==============================
           Header Actions
           (fixed: single source of truth for spacing — gap on the
           flex container instead of a hardcoded margin on .cart-btn)
           ============================== */
        .header-actions {
            display: flex;
            align-items: center;
        ...
        /* subtle divider so search and cart read as two separate
           groups instead of just floating apart */
        .search-box::after {
            content: '';
            position: absolute;
            right: -11px;
    ...
    <header class="header">
        <div class="header-container">
            ...
            <div class="header-actions">
                ...
                @php
                    $isOwner = auth()->check() && (auth()->user()->role === 'owner' || auth()->user()->role === 'admin');
                    $customerId = session('customer_id');
                    $customer = $customerId ? \App\Models\Customer::find($customerId) : null;
                    $isCustomer = session('customer_logged_in') || $customer;

                    if ($isOwner) {
                        $userDisplayName = auth()->user()->name ?? 'เจ้าของร้าน';
                        $userRoleLabel = 'เจ้าของร้าน / ผู้ดูแลระบบ';
                        $userInitial = mb_substr($userDisplayName, 0, 1);
                    } elseif ($isCustomer) {
                        $userDisplayName = $customer
                            ? trim(($customer->first_name ?? '') . ' ' . ($customer->last_name ?? ''))
                            : (session('customer_name') ?: 'ลูกค้า');
                        if (empty(trim($userDisplayName))) {
                            $userDisplayName = 'ลูกค้า KYRIX';
                        }
                        $userRoleLabel = 'สมาชิก KYRIX';
                        $userInitial = mb_substr($userDisplayName, 0, 1);
                    } else {
                        ...
                        <div class="dropdown-menu" id="userDropdown">
                            ...
                            <div class="profile-dropdown-header">
                                ...
                                <div style="min-width: 0;">
                                    ...
                                    <div class="profile-dropdown-role">{{ $userRoleLabel }}</div>
                                </div>
                            </div>
                            ...
                            @if ($isOwner)
                                ...
                                @if (Route::has('owner.bookings.index'))
                                    <a href="{{ route('owner.bookings.index') }}" class="dropdown-item">
                                        <i class="fa-solid fa-calendar-check"></i>
                                        จัดการการจอง
                                    </a>
                                @endif
                                @if (Route::has('owner.dresses.index'))
                                    <a href="{{ route('owner.dresses.index') }}" class="dropdown-item">
                            ...
                            <form action="{{ route('logout') }}" method="POST" style="margin: 0;">
                                ...
                                <button type="submit" class="dropdown-item logout-item"
                                    style="width: 100%; border: none; background: none; cursor: pointer; text-align: left; font-family: inherit; font-size: 14px;"
        ...
        <div class="footer-container">
            ...
            <div class="footer-col">
                ...
                <p style="margin-top: 14px; max-width: 320px;">
                    ...
                    บริการเช่าชุดสายฝอ สายหวาน ชุดออกงาน
                    เกรดพรีเมียม
            ...
            <div class="footer-col">
                <h4>ประเภทชุด</h4>
                <ul class="footer-links">
                    <li><a href="{{ route('products.index') }}?category_id=1">ชุดสายฝอ</a></li>
                    <li><a href="{{ route('products.index') }}?category_id=2">ชุดสายหวาน</a></li>
                    <li><a href="{{ route('products.index') }}?category_id=3">ชุดออกงาน</a></li>
                    <li><a href="{{ route('products.index') }}?category_id=4">มินิเดรส</a></li>
                    <li><a href="{{ route('products.index') }}?category_id=6">เสื้อ & ท็อปส์</a></li>
                    <li><a href="{{ route('products.index') }}?category_id=6">กระโปรง</a></li>
                    <li><a href="{{ route('products.index') }}?category_id=6">กางเกง</a></li>
                </ul>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (nav && indicator && links.length) {
                ...
                if (hash) {
                    const hashLink = document.querySelector(`.nav-links a[data-hash="${hash.substring(1)}"]`);
                    if (hashLink) {
                        activeLink = hashLink;
                    }
                }
                ...
                links.forEach(function(link) {
                    link.addEventListener('click', function(e) {
                        ...
                        if (this.classList.contains('js-hash-link') && href.includes('#')) {
                            const isSamePage =
                            ...
                            if (isSamePage) {
                                ...
                                const hashValue = this.dataset.hash;
                ...
                window.addEventListener('popstate', function() {
                    ...
                    if (currentHash) {
                        const hashLink = document.querySelector(`.nav-links a[data-hash="${currentHash.substring(1)}"]`);
                        if (hashLink) {
                            setActive(hashLink);
                            return;
                        }
                    ...
                    const homeLink = document.querySelector('.nav-links a[href="{{ route('home') }}"]');
                    if (homeLink) {
                        setActive(homeLink);
                    }
                });
            ...
            if (userMenu && userTrigger) {
                userTrigger.addEventListener('click', function(e) {
                    ...
                    userTrigger.setAttribute('aria-expanded', isExpanded ? 'true' : 'false');
                });
