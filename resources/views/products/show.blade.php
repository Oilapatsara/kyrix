...
<div class="breadcrumb-wrap">
    <div class="breadcrumb">
        ...
        <span>/</span>
        <a href="{{ route('products.index', ['category_id' => $product->category_id]) }}">{{ $product->category->category_name ?? 'หมวดหมู่' }}</a>
        <span>/</span>
        ...
        @if($product->images->count() > 1)
            <div class="thumbs-row">
                @foreach($product->images as $idx => $img)
                    @php
                        $thumbUrl = str_starts_with($img->image_path, 'http') ? $img->image_path : asset('storage/' . $img->image_path);
                    @endphp
                    <button type="button" class="thumb-btn {{ $idx === 0 ? 'active' : '' }}" onclick="switchImage('{{ $thumbUrl }}', this)">
    ...
    <div class="product-info">
        <div class="info-header">
            ...
            <div class="info-meta-row" style="flex-wrap: wrap; gap: 14px;">
                ...
                <span><i class="fa-solid fa-boxes-stacked"></i> จำนวนชุดที่มี: <strong>{{ $product->stock }}</strong> ชุด</span>
                <span>
                    @if($product->status === 'available')
                        <span class="badge badge-success"><i class="fa-solid fa-circle-check"></i> ว่าง (พร้อมเช่า)</span>
        ...
        <div class="price-box">
            ...
            <div>
                <span class="deposit-badge"><i class="fa-solid fa-shield"></i> เงินมัดจำ ฿{{ number_format($product->deposit) }}</span>
            </div>
        </div>
        ...
        <div>
            ...
            <p style="font-size: 14px; color: #555; line-height: 1.8; margin-bottom: 12px;">{{ $product->description }}</p>
            <div style="display: flex; gap: 20px; font-size: 13px; color: var(--text-muted);">
                <span><i class="fa-solid fa-ruler"></i> ไซซ์มาตรฐาน: <strong>{{ $product->size ?? 'M' }}</strong></span>
        ...
        <div class="rental-form-card">
                ...
                <form action="{{ route('rentals.book', $product->product_id) }}" method="POST" id="rentalBookingForm">
                    ...
                    <div class="form-group">
                        ...
                        <div class="options-row">
                            @foreach($product->sizes_list as $idx => $s)
                                <button type="button" class="option-btn {{ $idx === 0 ? 'selected' : '' }}" onclick="selectSize('{{ $s }}', this)">
                                    {{ $s }}
                                </button>
                            @endforeach
                        </div>
                        <input type="hidden" name="size" id="selectedSize" value="{{ $product->sizes_list[0] ?? 'M' }}">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="options-row">
                            @foreach($product->colors_list as $idx => $c)
                                <button type="button" class="option-btn {{ $idx === 0 ? 'selected' : '' }}" onclick="selectColor('{{ $c }}', this)">
                                    {{ $c }}
                                </button>
                            @endforeach
                        </div>
                        <input type="hidden" name="color" id="selectedColor" value="{{ $product->colors_list[0] ?? $product->color }}">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="dates-grid">
                            <div>
                                <span style="font-size: 12px; color: var(--text-muted); display: block; margin-bottom: 4px;">วันที่เริ่มเช่า (Start Date)</span>
                                <input type="date" name="start_date" id="startDate" class="date-input" value="{{ date('Y-m-d') }}" min="{{ date('Y-m-d') }}" required onchange="calculateRental()">
                            </div>
                            <div>
                                <span style="font-size: 12px; color: var(--text-muted); display: block; margin-bottom: 4px;">วันที่คืนชุด (End Date)</span>
                                <input type="date" name="end_date" id="endDate" class="date-input" value="{{ date('Y-m-d', strtotime('+2 days')) }}" min="{{ date('Y-m-d') }}" required onchange="calculateRental()">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div style="display: flex; align-items: center; gap: 10px;">
                            <input type="number" name="quantity" id="rentalQty" value="1" min="1" max="{{ $product->stock }}" class="date-input" style="width: 110px;" required onchange="calculateRental()">
                            <span style="font-size: 13px; color: var(--text-muted);">(มีในสต็อกทั้งหมด {{ $product->stock }} ชุด)</span>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="note" class="date-input" placeholder="เช่น ขอปรับขนาดเอวเข้า 1 นิ้ว, รับชุดช่วงบ่าย">
                    </div>
                    ...
                    <div class="live-calc-box">
                        ...
                        <div class="calc-row">
                            <span>ค่าเช่าชุด (฿{{ number_format($product->rental_price) }} x <span id="displayDaysText">3</span> วัน x <span id="displayQtyText">1</span> ชุด):</span>
                        ...
                        <div class="calc-row total">
                            ...
                            <span id="displayGrandTotal">฿{{ number_format(($product->rental_price * 3) + $product->deposit) }}</span>
                        </div>
                    </div>
            ...
            @else
                <div style="text-align: center; padding: 30px 10px; color: var(--text-muted);">
                    <i class="fa-solid fa-circle-exclamation" style="font-size: 38px; color: #dc2626; margin-bottom: 10px;"></i>
                    <h4 style="font-size: 16px; font-weight: 700; color: #b91c1c; margin-bottom: 6px;">ชุดนี้ไม่พร้อมให้เช่าในขณะนี้</h4>
                    <p style="font-size: 13px;">สถานะ: {{ $product->status_label }}</p>
                    <a href="{{ route('products.index') }}" class="btn btn-secondary btn-sm" style="margin-top: 12px;">เลือกดูชุดอื่นๆ</a>
                </div>
            @endif
        </div>
    </div>
...
<section class="reviews-section" id="reviews">
    ...
    @forelse($product->reviews as $review)
        <div class="review-item">
            <div class="review-user-row">
                <div style="display: flex; align-items: center; gap: 10px;">
                    <div style="width: 38px; height: 38px; border-radius: 50%; background: var(--primary-soft); color: var(--primary); display: flex; align-items: center; justify-content: center; font-weight: 700;">
                        {{ mb_substr($review->customer->user->name ?? 'U', 0, 1) }}
                    </div>
                    <div>
                        <div style="font-weight: 700; font-size: 14px;">{{ $review->customer->user->name ?? 'ลูกค้าสมาชิก' }}</div>
                        <div style="font-size: 12px; color: var(--text-muted);">{{ $review->created_at->format('d/m/Y') }}</div>
                    </div>
                </div>
            ...
            @if($review->image_path)
                @php
                    $revImg = str_starts_with($review->image_path, 'http') ? $review->image_path : asset($review->image_path);
                @endphp
                <div>
    ...
    @empty
        <div style="background: #fff; padding: 40px; text-align: center; border-radius: var(--radius-md); border: 1px solid var(--border); color: var(--text-muted);">
...
<script>
    ...
    function calculateRental() {
        ...
        if (document.getElementById('displayQtyDepositText')) document.getElementById('displayQtyDepositText').innerText = qty;
        if (document.getElementById('displayRentalSubtotal')) document.getElementById('displayRentalSubtotal').innerText = '฿' + rentalSubtotal.toLocaleString();
        if (document.getElementById('displayDeposit')) document.getElementById('displayDeposit').innerText = '฿' + depositTotal.toLocaleString();
        if (document.getElementById('displayGrandTotal')) document.getElementById('displayGrandTotal').innerText = '฿' + grandTotal.toLocaleString();
    }
